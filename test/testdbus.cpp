...
// Test that MethodCallDefer allows a D-Bus method to defer its reply until
// an async operation (e.g. another D-Bus call) completes.
class DeferTestObject : public ObjectVTable<DeferTestObject> {
    std::string testDefer(const std::string &input) {
        // Capture enough to build the deferred reply.
        auto slotHolder = std::make_shared<std::unique_ptr<Slot>>();
        auto *bus = this->bus();

        throw MethodCallDefer([input, slotHolder, bus](Message originalMsg) {
            // Make an async D-Bus call (introspect ourselves) just to exercise
            // the async path; the return value is ignored.
            auto callMsg =
